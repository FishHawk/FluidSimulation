    ...
    for (int i = 0; i < fluid_particles_number_; i++) {
        ...
        for (const auto &neighbor : neighbors[cell_index]) {
            density += particles_.masses[neighbor] *
                       Kernel::poly6_kernel(particles_.predicted_positions[i] - particles_.predicted_positions[neighbor], sph_radius_);
        }
        ...
        if (constraint != 0.0) {
            ...
            for (const auto &neighbor : neighbors[cell_index]) {
                glm::vec3 grad_cj = static_cast<float>(particles_.masses[neighbor] / fluid_density_) *
                                    Kernel::poly6_kernal_grade(particles_.predicted_positions[i] - particles_.predicted_positions[neighbor], sph_radius_);
